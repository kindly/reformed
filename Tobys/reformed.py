...
tables = sa.Table("table_", dbconfig.metadata,
...
table_param = sa.Table("table_param", dbconfig.metadata,
               sa.Column( 'id' ,   sa.Integer,    primary_key=True),    
               sa.Column('table_id', sa.Integer, sa.ForeignKey("table_.id")),
...
class Table(object):
   ...
   def paramset(self):
       ...
       try:	# we need to check if the table is already entered - new columns? what to do
...
class Database(object):
   ...
   def __init__ (self):
       ...
       # get a new session for creating the database
       # this prevents us running into caused by errors from the dodgy hack
       # in class Table() due to putting duplicate rows in we have a polluted session
       # it may be best to open and close the sessions a bit better in this module
