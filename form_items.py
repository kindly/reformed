...
def make_item(form_item = None, prefix = None, form_type = None, template_name = None, data = None):
	...
	# set defaults
	for key in data_defaults.keys():
		if not data.has_key(key):
			data[key] = data_defaults[key]

	# look up the form item we want
	if templates.has_key(template_name):
		out = Template(templates[template_name]).safe_substitute(data)	
	elif functions.has_key(template_name):
		out = functions[template_name](form_item, data)
	else:
	...
	if form_type == "grid":
		...
		if data.has_key('label'):
			label = Template(templates['label']).safe_substitute(data)
			return "\n<p>%s%s</p>" % (label, out)
		else:
			return "\n<p>%s</p>" % out
